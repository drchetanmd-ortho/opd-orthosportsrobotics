...
const DB = {
  async init() {
    return new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = e => {
        ...
        // Appointments (future bookings shown on the calendar)
        if (!db.objectStoreNames.contains('appointments')) {
          const ap = db.createObjectStore('appointments', { keyPath: 'id' });
          ap.createIndex('date', 'date', { unique: false });
          ap.createIndex('patientId', 'patientId', { unique: false });
        }
      };
  ...
  async saveAppointment(appt) {
    return new Promise((resolve, reject) => {
      ...
      const req = tx.objectStore('appointments').put(appt);
      req.onsuccess = () => resolve(appt);
      req.onerror = e => reject(e.target.error);
    });
  },
  ...
  async deleteAppointment(id) {
    return new Promise((resolve, reject) => {
      const tx = db.transaction('appointments', 'readwrite');
      const req = tx.objectStore('appointments').delete(id);
  ...
  async deletePatient(id) {
    // Fetch data first (read-only, outside the write transaction)
    const patient = await this.getPatient(id);
    if (!patient) return;
    const visits = await this.getPatientVisits(id);
    const entry = { id, patient, visits, deletedAt: Date.now() };

    // Single transaction across all 3 stores — atomic: all succeed or all fail
    return new Promise((resolve, reject) => {
      ...
      tx.objectStore('recycle_bin').put(entry);
